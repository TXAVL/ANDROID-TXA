        ...
        override fun run() {
            try {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    ...
                    // Kiểm tra foreground notification
                    val hasBackgroundNotification = activeNotifications.any { it.id == NOTIFICATION_ID_BACKGROUND }
                    if (!hasBackgroundNotification) {
                        ...
                        android.util.Log.d("UpdateCheckService", "Background notification bị xóa, tự động tạo lại (hide=$hideNotification)")
                        logWriter.writeUpdateCheckLog("Background notification bị xóa, tự động tạo lại (hide=$hideNotification)", "INFO")
                        createForegroundNotification(hideNotification = hideNotification)
                    }
                    
                    // Kiểm tra update notification - nếu có update và notification bị xóa thì tạo lại
                    val hasUpdateNotification = activeNotifications.any { it.id == NotificationHelper.NOTIFICATION_ID_UPDATE }
                    if (!hasUpdateNotification && lastNotifiedVersion.isNotEmpty()) {
                        // Update notification bị xóa, kiểm tra lại và tạo lại nếu vẫn có update
                        android.util.Log.d("UpdateCheckService", "Update notification bị xóa, kiểm tra lại và tạo lại nếu cần")
                        logWriter.writeUpdateCheckLog("Update notification bị xóa, kiểm tra lại và tạo lại nếu cần", "INFO")
                        
                        // Kiểm tra lại update
                        updateChecker.checkUpdate { updateInfo ->
                            handler.post {
                                if (updateInfo != null && updateInfo.versionName == lastNotifiedVersion) {
                                    // Vẫn có update cùng version, tạo lại notification
                                    try {
                                        notificationHelper.showUpdateNotification(
                                            versionName = updateInfo.versionName,
                                            downloadUrl = updateInfo.downloadUrl,
                                        ...
                                        android.util.Log.d("UpdateCheckService", "Đã tạo lại update notification cho version ${updateInfo.versionName}")
                                        logWriter.writeUpdateCheckLog("Đã tạo lại update notification cho version ${updateInfo.versionName}", "INFO")
                                    } catch (e: Exception) {
                                        android.util.Log.e("UpdateCheckService", "Lỗi khi tạo lại update notification", e)
                                        logWriter.writeUpdateCheckLog("Lỗi khi tạo lại update notification: ${e.message}", "ERROR")
                                    }
                                }
                            }
                        }
